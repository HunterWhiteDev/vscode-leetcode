...
export async function previewProblem(
  ...
  const descString: string = await leetCodeExecutor.getDescription(
    node.id,
    needTranslation
  );
...
export async function showSolution(
  ...
  try {
    const needTranslation: boolean =
      settingUtils.shouldUseEndpointTranslation();
    const solution: string = await leetCodeExecutor.showSolution(
      problemInput,
      language,
      needTranslation
    );
...
async function fetchProblemLanguage(): Promise<string | undefined> {
  ...
  let defaultLanguage: string | undefined =
    leetCodeConfig.get<string>("defaultLanguage");
  ...
  (async (): Promise<void> => {
    if (
      language &&
      !defaultLanguage &&
      leetCodeConfig.get<boolean>("hint.setDefaultLanguage")
    ) {
...
async function showProblemInternal(node: IProblem): Promise<void> {
  try {
    ...
    const descriptionConfig: IDescriptionConfiguration =
      settingUtils.getDescriptionConfiguration();
    const needTranslation: boolean =
      settingUtils.shouldUseEndpointTranslation();

    await leetCodeExecutor.showProblem(
      node,
      language,
      finalPath,
...
async function showDescriptionView(node: IProblem): Promise<void> {
  return previewProblem(
    ...
    vscode.workspace
      ...
      .get<boolean>("enableSideMode", true)
  );
}
async function parseProblemsToPicks(
  ...
  return new Promise(
    async (
      resolve: (res: Array<IQuickItemEx<IProblem>>) => void
    ): Promise<void> => {
      const picks: Array<IQuickItemEx<IProblem>> = (await p).map(
        (problem: IProblem) =>
          Object.assign(
            {},
            {
              label: `${parseProblemDecorator(problem.state, problem.locked)}${
                problem.id
              }.${problem.name}`,
              description: "",
